...
一个功能全面的 Web 端 3D 场景编辑器，基于 Babylon.js 7.x 引擎开发，支持 WebGPU 和 WebGL 渲染。
...
- 模型导入: 前端基于 `@babylonjs/loaders` 支持本地 .babylon /.glb模型文件, 通过 `URL.createObjectURL` 与 Babylon `SceneLoader` 直接加载, 同时支持将文件拖拽到 3D 视图中导入
...
- **[新增]** 场景截图导出功能，支持在无 HDR 环境时自动应用透明背景;导出渲染时，可以选择倍数和设置分辨率。
- **[新增]** 新建摄像机后，可以多建几个几个摄像机切换不同视角;相机的参数增加了惯性 (阻尼感)、旋转灵敏度X 、旋转灵敏度 Y、缩放精度。
